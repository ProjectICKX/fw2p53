...
class Regex {
	 ...
	 */
	public static function Match ($pattern, $subject, &$matches = array(), $flags = 0, $offset = 0, $from_encoding = Encoding::DEFAULT_ENCODING) {
	 ...
	 * @return	mixed	マッチ回数 エラー時はbool false
	 */
	public static function MatchAll ($pattern, $subject, &$matches = array(), $flags = \PREG_PATTERN_ORDER, $offset = 0, $from_encoding = Encoding::DEFAULT_ENCODING) {
		//マルチバイトセーフにするため、強制的にPCRE8オプションを付与する
		$pattern = static::ForcePcre8Modifiers($pattern);
	...
	public static function ReplaceCallback ($pattern , callable $callback, $subject, $limit = -1, &$count = null, $from_encoding = Encoding::DEFAULT_ENCODING) {
		...
		//$matchesのエンコーディングを元に戻す
		$ret = Encoding::ConvertArrayRecursive(array($ret), $from_encoding, Encoding::UTF_8);
		$ret = $ret[0];
